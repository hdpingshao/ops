...
{
    ...
    limit_conn_zone $binary_remote_addr zone=pingsho:10m;
    ...
    server
    {
        ...
        limit_conn pingsho 10;
        ...   
    }
}
说明：首先用limit_conn_zone定义了一个内存区块索引pingsho，大小为10m，它以$binary_remote_addr作为key。
该配置只能在http里面配置，不支持在server里配置。

limit_conn 定义针对pingsho这个zone，并发连接为10个。在这需要注意一下，这个10指的是单个IP的并发最多为10个。
```
###### 2. 速度限制
```
location ~ /download/ {
...
这个阈值的默认值为0。如：
limit_req_zone $binary_remote_addr zone=pingsho:10m rate=1r/s;
server {
    location /upload/ {
        limit_req zone=pingsho burst=5;
    }
}
...
limit_req zone=pingsho burst=5 nodelay;
```
...
http {
    ...
    limit_req_zone $limit zone=pingsho:10m rate=1r/s;
